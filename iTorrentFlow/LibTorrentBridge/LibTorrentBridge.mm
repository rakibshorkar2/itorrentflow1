...
// LibTorrent C++ headers — requires LibTorrent to be in header search path
...
LTSessionRef lt_session_create(void) {
    ...
    pack.set_int(settings_pack::dht_announce_interval, 30 * 60); // 30 min
                ...
                if (auto *ta = alert_cast<torrent_alert>(a)) {
                    ...
                    if (auto *sa = alert_cast<state_changed_alert>(a)) {
                        if (s->statusCb) {
                            ...
                            s->statusCb(ih.c_str(), st.progress,
                    ...
                    if (auto *pa = alert_cast<piece_finished_alert>(a)) {
                        if (s->pieceCb) {
                            s->pieceCb(ih.c_str(), pa->piece_index);
                        }
                    }
                    ...
                    if (auto *fa = alert_cast<torrent_finished_alert>(a)) {
                        if (s->statusCb) {
                            auto st = ta->handle.status();
                            s->statusCb(ih.c_str(), 1.0,
                                        st.download_rate, st.upload_rate,
                                        st.num_peers, st.num_seeds,
                                        (int)st.state, "Completed");
                        }
                    }
                }
...
void lt_session_set_listen_port(LTSessionRef session, int port) {
    auto *s = get_session(session);
    settings_pack pack;
    pack.set_int(settings_pack::listen_port, port);
    s->ses.apply_settings(std::move(pack));
}
...
void lt_session_set_max_connections(LTSessionRef session, int max) {
    auto *s = get_session(session);
    settings_pack pack;
    pack.set_int(settings_pack::connections_limit, max);
    s->ses.apply_settings(std::move(pack));
}
...
void lt_session_set_max_upload_rate(LTSessionRef session, int64_t bytesPerSec) {
    auto *s = get_session(session);
    settings_pack pack;
    pack.set_int(settings_pack::upload_rate_limit, (int)bytesPerSec);
    s->ses.apply_settings(std::move(pack));
}
...
void lt_session_set_max_download_rate(LTSessionRef session, int64_t bytesPerSec) {
    auto *s = get_session(session);
    settings_pack pack;
    pack.set_int(settings_pack::download_rate_limit, (int)bytesPerSec);
    s->ses.apply_settings(std::move(pack));
}
...
void lt_session_enable_dht(LTSessionRef session, bool enable) {
    auto *s = get_session(session);
    settings_pack pack;
    pack.set_bool(settings_pack::enable_dht, enable);
    s->ses.apply_settings(std::move(pack));
}
...
void lt_session_enable_lsd(LTSessionRef session, bool enable) {
    auto *s = get_session(session);
    settings_pack pack;
    pack.set_bool(settings_pack::enable_lsd, enable);
    s->ses.apply_settings(std::move(pack));
}
...
void lt_session_enable_upnp(LTSessionRef session, bool enable) {
    auto *s = get_session(session);
    settings_pack pack;
    pack.set_bool(settings_pack::enable_upnp, enable);
    s->ses.apply_settings(std::move(pack));
}
...
void lt_session_enable_natpmp(LTSessionRef session, bool enable) {
    auto *s = get_session(session);
    settings_pack pack;
    pack.set_bool(settings_pack::enable_natpmp, enable);
    s->ses.apply_settings(std::move(pack));
}
...
void lt_session_remove(LTSessionRef session, const char *infoHash, bool deleteFiles) {
    ...
    auto it = s->handles.find(infoHash);
    if (it != s->handles.end()) {
        s->ses.remove_torrent(it->second,
            deleteFiles ? session::delete_files : session::none);
        s->handles.erase(it);
    }
}
...
void lt_session_set_file_priority(LTSessionRef session, const char *infoHash,
    ...
    if (it != s->handles.end()) {
        ...
        if (fileIndex >= 0 && fileIndex < (int)priorities.size()) {
            priorities[fileIndex] = (download_priority_t)priority;
...
void lt_session_add_tracker(LTSessionRef session, const char *infoHash,
    ...
    auto it = s->handles.find(infoHash);
    if (it != s->handles.end()) {
        announce_entry entry(trackerURL);
        it->second.add_tracker(entry);
    }
}
...
void lt_session_replace_trackers(LTSessionRef session, const char *infoHash,
    ...
    if (it != s->handles.end()) {
        ...
        for (int i = 0; i < count; i++) {
            entries.emplace_back(trackers[i]);
        }
...
void lt_session_add_dht_node(LTSessionRef session, const char *host, int port) {
    auto *s = get_session(session);
    s->ses.add_dht_node(std::make_pair(std::string(host), port));
}

void lt_session_add_dht_router(LTSessionRef session, const char *host, int port) {
    auto *s = get_session(session);
    s->ses.add_dht_router(std::make_pair(std::string(host), port));
}
